...
impl MigrationTrait for CreateTestProductTable {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        let table = Table::create()
            ...
            .col(ColumnDef::new(TestProductColumn::Price).integer().not_null())
...
async fn test_real_api_list_endpoint_optimization() {
    ...
    assert!(!product.id.to_string().is_empty(), "List model should contain id");
    assert_eq!(product.name, "Test Product", "List model should contain name");
    assert_eq!(product.price, 1999, "List model should contain price");
    assert!(product.description.is_some(), "List model should contain description");
