...
impl MigrationTrait for CreateBlogPostTable {
    async fn up(&self, manager: &SchemaManager) -> Result<(), sea_orm::DbErr> {
        let table = Table::create()
            ...
            .if_not_exists()
            .col(
                ColumnDef::new(BlogPostColumn::Id)
                    .uuid()
                    .not_null()
                    .primary_key(),
            )
            ...
            .col(
                ColumnDef::new(BlogPostColumn::Published)
                    .boolean()
                    .not_null()
                    .default(false),
            )
            .col(
                ColumnDef::new(BlogPostColumn::ViewCount)
                    .integer()
                    .not_null()
                    .default(0),
            )
            .col(
                ColumnDef::new(BlogPostColumn::CreatedAt)
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), sea_orm::DbErr> {
        manager
            .drop_table(Table::drop().table(BlogPostEntity).to_owned())
            .await?;
        Ok(())
    }
}
...
async fn test_combined_filter_and_fulltext() {
    ...
    for post in &posts {
        ...
        assert!(
            ...
            "Post should contain search terms: {}",
            post.title
        );
    }
}
...
async fn test_fulltext_with_pagination() {
    ...
    assert!(
        ...
        "Should include Content-Range header for pagination"
    );
    ...
    assert!(
        posts.len() <= 2,
        "Should return at most 2 posts due to pagination"
    );
}
...
async fn test_fulltext_search_no_results() {
    ...
    assert_eq!(
        posts.len(),
        0,
        "Should return no posts for nonexistent search term"
    );
}
