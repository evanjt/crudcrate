...
async fn test_filter_score_gte_float_50() {
    ...
    assert_eq!(tasks.len(), 3, "Should find 3 tasks with score >= 50.0 (Medium=50.0, High=95.7, Boundary=75.0), found {}", tasks.len());
}
...
async fn test_filter_score_gte_float_25() {
    ...
    for task in &tasks {
        assert!(task.score >= 25.0, "Task '{}' has score {} which is < 25.0", task.title, task.score);
    }
    assert_eq!(tasks.len(), 3, "Should find 3 tasks with score >= 25.0, found {}", tasks.len());
}
...
async fn test_filter_points_gte_integer_50() {
    ...
    assert_eq!(tasks.len(), 3, "Should find 3 tasks with points >= 50 (Medium=50, High=100, Boundary=75), found {}", tasks.len());
}
...
async fn test_filter_points_gte_integer_20() {
    ...
    for task in &tasks {
        assert!(task.points >= 20, "Task '{}' has points {} which is < 20", task.title, task.points);
    }
    assert_eq!(tasks.len(), 3, "Should find 3 tasks with points >= 20, found {}", tasks.len());
}
...
async fn test_filter_assignee_count_gte_small_integer_3() {
    ...
    assert_eq!(tasks.len(), 3, "Should find 3 tasks with assignee_count >= 3 (Medium=3, High=10, Boundary=5), found {}", tasks.len());
}
...
async fn test_filter_assignee_count_gte_small_integer_5() {
    ...
    for task in &tasks {
        assert!(task.assignee_count >= 5, "Task '{}' has assignee_count {} which is < 5", task.title, task.assignee_count);
    }
    assert_eq!(tasks.len(), 2, "Should find 2 tasks with assignee_count >= 5 (High=10, Boundary=5), found {}", tasks.len());
}
...
async fn test_filter_score_lte_float_50() {
    ...
    assert_eq!(tasks.len(), 3, "Should find 3 tasks with score <= 50.0 (Low=10.5, Medium=50.0, Zero=0.0), found {}", tasks.len());
}
...
async fn test_filter_score_lte_float_75() {
    ...
    for task in &tasks {
        assert!(task.score <= 75.0, "Task '{}' has score {} which is > 75.0", task.title, task.score);
    }
    assert_eq!(tasks.len(), 4, "Should find 4 tasks with score <= 75.0, found {}", tasks.len());
}
...
async fn test_filter_points_lte_integer_50() {
    ...
    assert_eq!(tasks.len(), 3, "Should find 3 tasks with points <= 50 (Low=5, Medium=50, Zero=0), found {}", tasks.len());
}
...
async fn test_filter_points_lte_integer_75() {
    ...
    for task in &tasks {
        assert!(task.points <= 75, "Task '{}' has points {} which is > 75", task.title, task.points);
    }
    assert_eq!(tasks.len(), 4, "Should find 4 tasks with points <= 75, found {}", tasks.len());
}
...
async fn test_filter_score_gt_float_50() {
    ...
    assert_eq!(tasks.len(), 2, "Should find 2 tasks with score > 50.0 (High=95.7, Boundary=75.0), found {}", tasks.len());
}
...
async fn test_filter_points_gt_integer_50() {
    ...
    assert_eq!(tasks.len(), 2, "Should find 2 tasks with points > 50 (High=100, Boundary=75), found {}", tasks.len());
}
...
async fn test_filter_points_gt_integer_5() {
    ...
    for task in &tasks {
        assert!(task.points > 5, "Task '{}' has points {} which is <= 5", task.title, task.points);
    }
    assert_eq!(tasks.len(), 3, "Should find 3 tasks with points > 5, found {}", tasks.len());
}
...
async fn test_filter_score_lt_float_50() {
    ...
    assert_eq!(tasks.len(), 2, "Should find 2 tasks with score < 50.0 (Low=10.5, Zero=0.0), found {}", tasks.len());
}
...
async fn test_filter_score_lt_float_25() {
    ...
    assert_eq!(tasks.len(), 2, "Should find 2 tasks with score < 25.0, found {}", tasks.len());
}
...
async fn test_filter_assignee_count_lt_small_integer_3() {
    ...
    assert_eq!(tasks.len(), 2, "Should find 2 tasks with assignee_count < 3 (Low=1, Zero=0), found {}", tasks.len());
}
...
async fn test_filter_assignee_count_lt_small_integer_5() {
    ...
    assert_eq!(tasks.len(), 3, "Should find 3 tasks with assignee_count < 5, found {}", tasks.len());
}
...
async fn test_filter_score_neq_float_0() {
    ...
    for task in &tasks {
        assert!(task.score != 0.0, "Task '{}' has score {} which equals 0.0", task.title, task.score);
    }
...
async fn test_filter_points_neq_integer_100() {
    ...
    for task in &tasks {
        assert!(task.points != 100, "Task '{}' has points {} which equals 100", task.title, task.points);
    }
    assert_eq!(tasks.len(), 4, "Should find 4 tasks with points != 100, found {}", tasks.len());
}
...
async fn test_filter_assignee_count_neq_small_integer_3() {
    ...
    for task in &tasks {
        assert!(task.assignee_count != 3, "Task '{}' has assignee_count {} which equals 3", task.title, task.assignee_count);
    }
    assert_eq!(tasks.len(), 4, "Should find 4 tasks with assignee_count != 3, found {}", tasks.len());
}
...
async fn test_filter_assignee_count_neq_small_integer_10() {
    ...
    assert_eq!(tasks.len(), 4, "Should find 4 tasks with assignee_count != 10, found {}", tasks.len());
}
...
async fn test_filter_completed_neq_boolean_true() {
    ...
    for task in &tasks {
        assert!(!task.completed, "Task '{}' should not be completed (completed != true)", task.title);
    }
    assert_eq!(tasks.len(), 3, "Should find 3 incomplete tasks (completed != true), found {}", tasks.len());
}
...
async fn test_filter_score_range_gte_25_lte_75() {
    ...
    for task in &tasks {
        assert!(task.score >= 25.0 && task.score <= 75.0, 
            "Task '{}' has score {} which is not between 25.0 and 75.0", task.title, task.score);
    }
    assert_eq!(tasks.len(), 2, "Should find 2 tasks with score between 25.0 and 75.0, found {}", tasks.len());
}
...
async fn test_filter_assignee_count_range_gte_1_lte_5() {
    let app = setup_app_with_numeric_test_data().await;
    let tasks = filter_tasks_with_comparison(&app, "{\"assignee_count_gte\":1,\"assignee_count_lte\":5}").await;
    
    for task in &tasks {
        assert!(task.assignee_count >= 1 && task.assignee_count <= 5, 
            "Task '{}' has assignee_count {} which is not between 1 and 5", task.title, task.assignee_count);
    }
    assert_eq!(tasks.len(), 3, "Should find 3 tasks with assignee_count between 1 and 5, found {}", tasks.len());
}
...
async fn test_filter_estimated_hours_gte_nullable_10() {
    ...
    assert_eq!(tasks.len(), 2, "Should find 2 tasks with estimated_hours >= 10.0 (High=40.0, Boundary=20.0), found {}", tasks.len());
}
...
async fn test_filter_estimated_hours_lte_nullable_15() {
    ...
    for task in &tasks {
        match task.estimated_hours {
            Some(hours) => assert!(hours <= 15.0, 
                "Task '{}' has estimated_hours {} which is > 15.0", task.title, hours),
            None => panic!("Task '{}' has null estimated_hours, should be filtered out", task.title),
        }
    }
    assert_eq!(tasks.len(), 2, "Should find 2 tasks with estimated_hours <= 15.0 (Low=1.0, Medium=8.0), found {}", tasks.len());
}
...
async fn test_filter_score_gt_zero_boundary() {
    ...
    assert_eq!(tasks.len(), 4, "Should find 4 tasks with score > 0.0 (excluding zero), found {}", tasks.len());
}
...
async fn test_filter_points_gte_zero_boundary() {
    ...
    for task in &tasks {
        assert!(task.points >= 0, "Task '{}' has points {} which is < 0", task.title, task.points);
    }
    assert_eq!(tasks.len(), 5, "Should find all 5 tasks with points >= 0, found {}", tasks.len());
}
...
async fn test_filter_invalid_comparison_operator() {
    ...
    assert!(tasks.len() <= 5, "Should not return more tasks than exist, found {}", tasks.len());
}
...
async fn test_filter_multiple_operators_score_gte_points_lte() {
    ...
    assert_eq!(tasks.len(), 2, "Should find 2 tasks matching both conditions, found {}", tasks.len());
}

#[tokio::test]
async fn test_filter_multiple_operators_complex_combination() {
    let app = setup_app_with_numeric_test_data().await;
    let tasks = filter_tasks_with_comparison(&app, "{\"score_gte\":40.0,\"points_lte\":80,\"assignee_count_neq\":10}").await;
    ...
    assert_eq!(tasks.len(), 2, "Should find 2 tasks matching all three conditions, found {}", tasks.len());
}