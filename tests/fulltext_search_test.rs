...
use crudcrate::{traits::CRUDResource, filter::apply_filters};
use crudcrate_derive::EntityToModels;
use sea_orm::{entity::prelude::*, DatabaseBackend};
use uuid::Uuid;
...
async fn test_fulltext_search_with_different_data_types() {
    ...
    for term in search_terms {
        ...
        assert!(!condition.is_empty(), "Condition should not be empty for term: {}", term);
    }
}
...
async fn test_no_fulltext_columns_fallback() {
    ...
    assert_eq!(SimpleItem::fulltext_searchable_columns().len(), 0, 
