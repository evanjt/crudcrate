...
async fn test_string_substring_matching_default() {
    ...
    assert_eq!(tasks.len(), 2, "Should find 2 tasks containing 'Alpha' in title, found {}", tasks.len());
...
async fn test_string_exact_matching_with_eq() {
    ...
    assert_eq!(tasks.len(), 1, "Should find exactly 1 task with title='Alpha', found {}", tasks.len());
    assert_eq!(tasks[0].title, "Alpha", "Found task should have exact title 'Alpha'");
}
...
async fn test_string_exact_matching_with_eq_full_title() {
    ...
    assert_eq!(tasks.len(), 1, "Should find exactly 1 task with full title, found {}", tasks.len());
    assert_eq!(tasks[0].title, "Alpha Work Project", "Found task should have exact full title");
}
...
async fn test_string_exact_matching_no_results() {
    ...
    assert_eq!(tasks.len(), 0, "Should find no tasks with non-existent exact title, found {}", tasks.len());
}
...
async fn test_description_substring_vs_exact() {
    ...
    assert_eq!(substring_tasks.len(), 2, "Substring search should find 2 tasks with 'Alpha' in description");
    
    // Test exact matching on description
    let exact_tasks = filter_tasks_by_string(&app, "{\"description_eq\":\"Just Alpha with no additional text\"}").await;
    assert_eq!(exact_tasks.len(), 1, "Exact search should find 1 task with exact description");
    assert_eq!(exact_tasks[0].description.as_ref().unwrap(), "Just Alpha with no additional text");
}
...
async fn test_enum_fields_still_use_exact_matching() {
    ...
    assert_eq!(no_tasks.len(), 0, "Should find no tasks with partial enum value 'Hi'");
}