...
async fn test_filter_boolean_completed_false() {
    ...
    assert_eq!(tasks.len(), 3, "Should find 3 incomplete tasks, found {}", tasks.len());
}
...
async fn test_filter_boolean_is_public_true() {
    ...
    assert_eq!(tasks.len(), 3, "Should find 3 public tasks, found {}", tasks.len());
}
...
async fn test_filter_enum_priority_high() {
    ...
    for task in &tasks {
        assert_eq!(task.priority, Priority::High, "Task '{}' should have High priority", task.title);
    }
    assert_eq!(tasks.len(), 1, "Should find 1 High priority task, found {}", tasks.len());
}
...
async fn test_filter_enum_priority_urgent() {
    ...
    assert_eq!(tasks.len(), 2, "Should find 2 Urgent priority tasks, found {}", tasks.len());
}
...
async fn test_filter_enum_priority_medium() {
    ...
    for task in &tasks {
        assert_eq!(task.priority, Priority::Medium, "Task '{}' should have Medium priority", task.title);
    }
    assert_eq!(tasks.len(), 1, "Should find 1 Medium priority task, found {}", tasks.len());
}
...
async fn test_filter_enum_priority_low() {
    ...
    assert_eq!(tasks.len(), 1, "Should find 1 Low priority task, found {}", tasks.len());
}
...
async fn test_filter_enum_status_done() {
    ...
    assert_eq!(tasks.len(), 2, "Should find 2 Done status tasks, found {}", tasks.len());
}
...
async fn test_filter_enum_status_inprogress() {
    ...
    for task in &tasks {
        assert_eq!(task.status, Status::InProgress, "Task '{}' should have InProgress status", task.title);
    }
    assert_eq!(tasks.len(), 1, "Should find 1 InProgress status task, found {}", tasks.len());
}
...
async fn test_filter_enum_status_todo() {
    ...
    for task in &tasks {
        assert_eq!(task.status, Status::Todo, "Task '{}' should have Todo status", task.title);
    }
    assert_eq!(tasks.len(), 1, "Should find 1 Todo status task, found {}", tasks.len());
}
...
async fn test_filter_enum_status_cancelled() {
    ...
    for task in &tasks {
        assert_eq!(task.status, Status::Cancelled, "Task '{}' should have Cancelled status", task.title);
    }
    assert_eq!(tasks.len(), 1, "Should find 1 Cancelled status task, found {}", tasks.len());
}
...
async fn test_filter_string_title_alpha() {
    ...
    for task in &tasks {
        assert!(task.title.contains("Alpha"), "Task '{}' should contain 'Alpha'", task.title);
    }
    assert_eq!(tasks.len(), 1, "Should find 1 Alpha title task, found {}", tasks.len());
}
...
async fn test_filter_string_title_work() {
    ...
    for task in &tasks {
        assert!(task.title.contains("Work"), "Task '{}' should contain 'Work'", task.title);
    }
    assert_eq!(tasks.len(), 1, "Should find 1 Work title task, found {}", tasks.len());
}
...
async fn test_filter_string_description_user() {
    ...
    for task in &tasks {
        assert!(task.description.as_ref().unwrap().contains("user"), 
            "Task '{}' should contain 'user' in description", task.title);
    }
    assert_eq!(tasks.len(), 1, "Should find 1 task with 'user' in description, found {}", tasks.len());
}
...
async fn test_filter_float_score_exact_67_25() {
    ...
    for task in &tasks {
        assert!((task.score - 67.25).abs() < f64::EPSILON, 
            "Task '{}' should have score 67.25, got {}", task.title, task.score);
    }
    assert_eq!(tasks.len(), 1, "Should find 1 task with score 67.25, found {}", tasks.len());
}
...
async fn test_filter_float_score_exact_zero() {
    ...
    for task in &tasks {
        assert!((task.score - 0.0).abs() < f64::EPSILON, 
            "Task '{}' should have score 0.0, got {}", task.title, task.score);
    }
    assert_eq!(tasks.len(), 1, "Should find 1 task with score 0.0, found {}", tasks.len());
}
...
async fn test_filter_integer_points_75() {
    ...
    for task in &tasks {
        assert_eq!(task.points, 75, "Task '{}' should have 75 points, got {}", task.title, task.points);
    }
    assert_eq!(tasks.len(), 1, "Should find 1 task with 75 points, found {}", tasks.len());
}
...
async fn test_filter_integer_points_25() {
    ...
    for task in &tasks {
        assert_eq!(task.points, 25, "Task '{}' should have 25 points, got {}", task.title, task.points);
    }
    assert_eq!(tasks.len(), 1, "Should find 1 task with 25 points, found {}", tasks.len());
}
...
async fn test_filter_small_integer_assignee_count_3() {
    ...
    for task in &tasks {
        assert_eq!(task.assignee_count, 3, "Task '{}' should have 3 assignees, got {}", task.title, task.assignee_count);
    }
    assert_eq!(tasks.len(), 1, "Should find 1 task with 3 assignees, found {}", tasks.len());
}
...
async fn test_filter_small_integer_assignee_count_5() {
    ...
    for task in &tasks {
        assert_eq!(task.assignee_count, 5, "Task '{}' should have 5 assignees, got {}", task.title, task.assignee_count);
    }
    assert_eq!(tasks.len(), 1, "Should find 1 task with 5 assignees, found {}", tasks.len());
}
...
async fn test_filter_nullable_estimated_hours_8_5() {
    ...
    for task in &tasks {
        match task.estimated_hours {
            Some(hours) => assert!((hours - 8.5).abs() < f32::EPSILON, 
                "Task '{}' should have 8.5 estimated_hours, got {}", task.title, hours),
            None => panic!("Task '{}' should have 8.5 estimated_hours, got null", task.title),
        }
    }
    assert_eq!(tasks.len(), 1, "Should find 1 task with 8.5 estimated_hours, found {}", tasks.len());
}
...
async fn test_filter_multi_completed_and_priority() {
    ...
    assert_eq!(tasks.len(), 1, "Should find 1 completed Urgent task, found {}", tasks.len());
}
...
async fn test_filter_multi_status_and_points() {
    ...
    for task in &tasks {
        assert_eq!(task.status, Status::Done, "Task '{}' should have Done status", task.title);
        assert_eq!(task.points, 50, "Task '{}' should have 50 points", task.title);
    }
    assert_eq!(tasks.len(), 1, "Should find 1 Done task with 50 points, found {}", tasks.len());
}
...
async fn test_filter_multi_is_public_and_assignee_count() {
    ...
    for task in &tasks {
        assert!(!task.is_public, "Task '{}' should be private", task.title);
        assert_eq!(task.assignee_count, 3, "Task '{}' should have 3 assignees", task.title);
    }
    assert_eq!(tasks.len(), 1, "Should find 1 private task with 3 assignees, found {}", tasks.len());
}
...
async fn test_filter_nonexistent_field() {
    ...
    assert_eq!(tasks.len(), 5, "Should return all 5 tasks for nonexistent field, found {}", tasks.len());
}
...
async fn test_filter_empty_string() {
    ...
    assert_eq!(tasks.len(), 0, "Should find no tasks with empty title, found {}", tasks.len());
}