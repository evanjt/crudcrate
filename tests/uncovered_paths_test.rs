...
fn test_generic_sort_with_empty_array() {
    ...
    let (_column, order) = generic_sort(Some("[]".to_string()), &sortable_columns, TodoColumn::Id);
...
fn test_generic_sort_with_none_sort() {
    ...
    let (_column, order) = generic_sort(None, &sortable_columns, TodoColumn::Id);
...
fn test_filter_options_schema_generation() {
    ...
    let deserialized: FilterOptions =
...
fn test_filter_options_schema_defaults() {
    ...
    let empty_json = "{}";
    let empty_options: FilterOptions =
