...
async fn setup_test_app_with_products() -> axum::Router {
    ...
    if _database_url.starts_with("postgres") {
        ...
        let _ = db.execute_unprepared("DROP TYPE IF EXISTS product_category CASCADE").await;
        db.execute_unprepared("CREATE TYPE product_category AS ENUM ('Electronics', 'Books', 'Clothing')")
            .await
            .expect("Failed to create PostgreSQL enum type");
    }

    // Create the products table using Sea-Query for database compatibility
...
async fn create_test_products(app: &axum::Router) {
    ...
    for product_data in products {
        ...
        if status != axum::http::StatusCode::CREATED {
            ...
            panic!("Failed to create product '{}'. Status: {}, Body: {}", product_data["name"], status, error_msg);
        }
    }
}
