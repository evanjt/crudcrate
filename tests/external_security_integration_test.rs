...
impl MigrationTrait for CreateSecurePostTable {
    async fn up(&self, manager: &SchemaManager) -> Result<(), sea_orm::DbErr> {
        let table = Table::create()
            ...
            .if_not_exists()
            .col(
                ColumnDef::new(SecurePostColumn::Id)
                    .uuid()
                    .not_null()
                    .primary_key(),
            )
            ...
            .col(
                ColumnDef::new(SecurePostColumn::Published)
                    .boolean()
                    .not_null()
                    .default(false),
            )
            .col(
                ColumnDef::new(SecurePostColumn::CreatedAt)
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), sea_orm::DbErr> {
        manager
            .drop_table(Table::drop().table(SecurePostEntity).to_owned())
            .await?;
        Ok(())
    }
}
...
async fn setup_secure_test_db() -> Result<DatabaseConnection, sea_orm::DbErr> {
    ...
    let sample_posts = vec![
        SecurePostCreate {
            ...
            content:
                "Learn how to secure your APIs using industry-standard libraries and practices."
                    .to_string(),
        ...
        SecurePostCreate {
            ...
            content: "Different approaches to implementing rate limiting in web applications."
                .to_string(),
...
async fn test_cors_headers_present() {
    ...
    assert!(
        response.status().is_success() || response.status() == axum::http::StatusCode::NO_CONTENT
    );
...
async fn test_fulltext_search_still_works_with_security() {
    ...
    assert!(
        found_secure_post,
        "Should find posts containing 'secure' or 'api'"
    );
}
