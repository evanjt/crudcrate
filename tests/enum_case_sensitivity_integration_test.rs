...
mod common;
use common::{setup_test_db_with_tasks, setup_task_app, task_entity::{Task, Priority, Model}};
...
async fn test_case_insensitive_vs_case_sensitive_enum_filtering() {
    ...
    let case_insensitive_tasks = filter_tasks_case_insensitive(&app, r#"{"priority":"high"}"#).await;
    assert_eq!(case_insensitive_tasks.len(), 1, "Case-insensitive: should find 1 task with 'high' priority");
    ...
    assert_eq!(exact_case_tasks.len(), 1, "Case-insensitive: should find 1 task with 'High' priority");
    
    // Should work with mixed case
    let mixed_case_tasks = filter_tasks_case_insensitive(&app, r#"{"priority":"HIGH"}"#).await;
    assert_eq!(mixed_case_tasks.len(), 1, "Case-insensitive: should find 1 task with 'HIGH' priority");
}

#[tokio::test]
async fn test_trait_method_configuration() {
    // Test that the trait methods return expected values
    assert!(!Task::enum_case_sensitive(), "Default Task should be case-insensitive");
    assert!(CaseSensitiveTask::enum_case_sensitive(), "CaseSensitiveTask should be case-sensitive");
}
