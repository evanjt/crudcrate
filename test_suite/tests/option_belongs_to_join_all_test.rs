...
async fn setup_test_db() -> Result<DatabaseConnection, DbErr> {
    ...
    // from belongs_to relations (obj_widget references obj_owner).
    for stmt in [
        Table::drop()
            .table(obj_widget::Entity)
            .if_exists()
            .to_owned(),
        Table::drop()
            .table(obj_owner::Entity)
            .if_exists()
            .to_owned(),
    ] {
    ...
    db.execute(backend.build(&schema.create_table_from_entity(obj_owner::Entity)))
        .await?;
    db.execute(backend.build(&schema.create_table_from_entity(obj_widget::Entity)))
        .await?;
