...
// Sensor readings migrator for aggregation tests
pub struct SensorReadingMigrator;

#[async_trait::async_trait]
impl MigratorTrait for SensorReadingMigrator {
    fn migrations() -> Vec<Box<dyn MigrationTrait>> {
        vec![Box::new(CreateSensorReadingTable)]
    }
}

pub struct CreateSensorReadingTable;

#[async_trait::async_trait]
impl MigrationName for CreateSensorReadingTable {
    fn name(&self) -> &'static str {
        "m20240101_000010_create_sensor_reading_table"
    }
}

#[async_trait::async_trait]
impl MigrationTrait for CreateSensorReadingTable {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        let table = Table::create()
            .table(sensor_reading::Entity)
            .if_not_exists()
            .col(
                ColumnDef::new(sensor_reading::Column::Id)
                    .uuid()
                    .not_null()
                    .primary_key(),
            )
            .col(
                ColumnDef::new(sensor_reading::Column::SiteId)
                    .uuid()
                    .not_null(),
            )
            .col(
                ColumnDef::new(sensor_reading::Column::RecordedAt)
                    .timestamp_with_time_zone()
                    .not_null(),
            )
            .col(
                ColumnDef::new(sensor_reading::Column::Value)
                    .double()
                    .not_null(),
            )
            .col(
                ColumnDef::new(sensor_reading::Column::CreatedAt)
                    .timestamp_with_time_zone()
                    .not_null(),
            )
            .to_owned();
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .drop_table(Table::drop().table(sensor_reading::Entity).to_owned())
            .await?;
        Ok(())
    }
}
...
#[allow(dead_code)]
pub async fn setup_sensor_db() -> Result<DatabaseConnection, DbErr> {
    let database_url = get_test_database_url();
    let db = Database::connect(&database_url).await?;
    SensorReadingMigrator::up(&db, None).await?;

    if !database_url.starts_with("sqlite::memory:") {
        let _ = db.execute_unprepared("DELETE FROM sensor_readings").await;
    }

    Ok(db)
}

// Readings migrator for aggregate-only tests (composite PK)
pub struct ReadingsMigrator;

#[async_trait::async_trait]
impl MigratorTrait for ReadingsMigrator {
    fn migrations() -> Vec<Box<dyn MigrationTrait>> {
        vec![Box::new(CreateReadingsTable)]
    }
}

pub struct CreateReadingsTable;

#[async_trait::async_trait]
impl MigrationName for CreateReadingsTable {
    fn name(&self) -> &'static str {
        "m20240101_000011_create_readings_table"
    }
}

#[async_trait::async_trait]
impl MigrationTrait for CreateReadingsTable {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        let table = Table::create()
            .table(reading::Entity)
            .if_not_exists()
            .col(
                ColumnDef::new(reading::Column::ParameterId)
                    .uuid()
                    .not_null(),
            )
            .col(
                ColumnDef::new(reading::Column::Time)
                    .timestamp_with_time_zone()
                    .not_null(),
            )
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .drop_table(Table::drop().table(reading::Entity).to_owned())
            .await?;
        Ok(())
    }
}
...
#[allow(dead_code)]
pub async fn setup_readings_db() -> Result<DatabaseConnection, DbErr> {
    let database_url = get_test_database_url();
    let db = Database::connect(&database_url).await?;
    ReadingsMigrator::up(&db, None).await?;

    if !database_url.starts_with("sqlite::memory:") {
        let _ = db.execute_unprepared("DELETE FROM readings").await;
    }
