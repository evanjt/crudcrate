...
async fn test_relationship_loading_in_get_all() {
    ...
    let customer_request = Request::builder()
        ...
        .uri("/customers")
        .header("content-type", "application/json")
        .body(Body::from(json!({"name": "Jane Smith", "email": "jane@example.com"}).to_string()))
        .unwrap();
    ...
    assert_eq!(found_customer.vehicles.len(), 1, "Customer get_all should include vehicles with join(all)");
