mod manufacturer__Model {
    ...
    impl crudcrate::CRUDResource for Manufacturer {
        type EntityType = Entity;
        type ColumnType = Column;
        type ActiveModelType = ActiveModel;
        type CreateModel = ManufacturerCreate;
        type UpdateModel = ManufacturerUpdate;
        type ListModel = ManufacturerList;
        ...
        const RESOURCE_NAME_SINGULAR: &'static str = "manufacturers";
        const RESOURCE_NAME_PLURAL: &'static str = "manufacturers";
        const TABLE_NAME: &'static str = "manufacturers";
        const RESOURCE_DESCRIPTION: &'static str = "This resource manages manufacturers items";
    ...
    impl From<Manufacturer> for ManufacturerList {
        fn from(model: Manufacturer) -> Self {
            Self {
                id: model.id,
                name: model.name,
            }
    ...
    impl From<Model> for ManufacturerList {
        fn from(model: Model) -> Self {
            Self {
                id: model.id,
                name: model.name,
    ...
    pub type ManufacturerScopedList = ManufacturerList;
    pub type ManufacturerScopedResponse = ManufacturerResponse;
    impl crudcrate::ScopeFilterable for ManufacturerList {}
    impl crudcrate::ScopeFilterable for Manufacturer {}
    #[derive(
        Clone,
        Debug,
        PartialEq,
...
mod vehicle__Model {
    ...
    impl crudcrate::CRUDResource for Vehicle {
        ...
        async fn get_one(
            ...
            let mut result = match main_model {
                Some(model) => {
                    let loaded_manufacturer = Box::pin(
                            model.find_related(super::manufacturer::Entity).one(db),
                        )
                        .await?
                        .map(|m: super::manufacturer::Model| super::manufacturer::Manufacturer::from(
                            m,
                        ));
                    let mut result: Self = model.into();
                    result.manufacturer = loaded_manufacturer;
                    result
                }
                None => {
                    return Err(
        ...
        async fn get_all(
            ...
            let parent_ids: Vec<crudcrate::PrimaryKeyType<Self>> = models
                ...
                .map(|m| m.id.clone())
                .collect();
            let mut manufacturer_by_parent: std::collections::HashMap<
                crudcrate::PrimaryKeyType<Self>,
            ...
            > = Box::pin(async {
                    ...
                    for parent_model in models.iter() {
                        let related = Box::pin(
                                parent_model
                                    .find_related(super::manufacturer::Entity)
                                    .one(db),
                            )
                            .await?
                            .map(super::manufacturer::Manufacturer::from);
                        if let Some(related) = related {
                            map.insert(parent_model.id.clone(), related);
                        }
                    }
            ...
            for model in models {
                let item = {
                    ...
                    item.manufacturer = manufacturer_by_parent.remove(&parent_id);
                    item
                };
    ...
    impl From<Vehicle> for VehicleList {
        fn from(model: Vehicle) -> Self {
            Self {
                ...
                manufacturer_id: model.manufacturer_id,
                make: model.make,
                manufacturer: model.manufacturer.map(Into::into),
            }
        }
    }
    ...
    #[doc(hidden)]
    pub const _BIDIRECTIONAL_RELATION_VEHICLE_MANUFACTURER: bool = crudcrate::impls!(
        super::manufacturer::Entity : sea_orm::Related < Entity >
    );
}
