mod Model {
    ...
    impl crudcrate::CRUDResource for Slug {
        type EntityType = Entity;
        type ColumnType = Column;
        type ActiveModelType = ActiveModel;
        type CreateModel = SlugCreate;
        ...
        const RESOURCE_DESCRIPTION: &'static str = "This resource manages slugs items";
        const FULLTEXT_LANGUAGE: &'static str = "english";
        fn sortable_columns() -> Vec<(&'static str, Self::ColumnType)> {
            vec![("title", Self::ColumnType::Title)]
        }
    ...
    impl From<Slug> for SlugList {
        fn from(model: Slug) -> Self {
            Self {
                id: model.id,
                title: model.title,
            }
    ...
    pub type SlugScopedList = SlugList;
    pub type SlugScopedResponse = SlugResponse;
    impl crudcrate::ScopeFilterable for SlugList {}
    impl crudcrate::ScopeFilterable for Slug {}
    #[derive(
        Clone,
        Debug,
        PartialEq,
    ...
    impl From<Slug> for SlugResponse {
        fn from(model: Slug) -> Self {
            Self {
                id: model.id,
                title: model.title,
