...
pub(crate) fn generate_delete_impl(crud_meta: &CRUDResourceMeta) -> proc_macro2::TokenStream {
    ...
    if let Some(ops_path) = &crud_meta.operations {
        return quote! {
            async fn delete<C: sea_orm::ConnectionTrait + sea_orm::TransactionTrait>(db: &C, id: crudcrate::PrimaryKeyType<Self>) -> Result<crudcrate::PrimaryKeyType<Self>, crudcrate::ApiError> {
                let ops = #ops_path;
...
pub(crate) fn generate_delete_many_impl(crud_meta: &CRUDResourceMeta) -> proc_macro2::TokenStream {
    ...
    if let Some(ops_path) = &crud_meta.operations {
        return quote! {
            async fn delete_many<C: sea_orm::ConnectionTrait + sea_orm::TransactionTrait>(db: &C, ids: Vec<crudcrate::PrimaryKeyType<Self>>) -> Result<Vec<crudcrate::PrimaryKeyType<Self>>, crudcrate::ApiError> {
                let ops = #ops_path;
