...
impl MigrationTrait for CreateBenchmarkTable {
    async fn up(&self, manager: &SchemaManager) -> Result<(), sea_orm::DbErr> {
        let table = Table::create()
            ...
            .if_not_exists()
            .col(ColumnDef::new(BenchmarkColumn::Id).uuid().not_null().primary_key())
            ...
            .col(ColumnDef::new(BenchmarkColumn::Published).boolean().not_null().default(false))
            .col(ColumnDef::new(BenchmarkColumn::Category).string().not_null())
            .col(ColumnDef::new(BenchmarkColumn::ViewCount).integer().not_null().default(0))
            .col(ColumnDef::new(BenchmarkColumn::Priority).integer().not_null().default(0))
            .col(ColumnDef::new(BenchmarkColumn::CreatedAt).timestamp_with_time_zone().not_null())
            .to_owned();
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), sea_orm::DbErr> {
        manager.drop_table(Table::drop().table(BenchmarkEntity).to_owned()).await?;
        Ok(())
    }
}
    ...
    async fn up(&self, manager: &SchemaManager) -> Result<(), sea_orm::DbErr> {
        ...
        match db.get_database_backend() {
            sea_orm::DatabaseBackend::Postgres => {
                ...
                manager
                    .get_connection()
                    .execute_unprepared("CREATE INDEX idx_benchmark_posts_published ON benchmark_posts (published)")
                    .await?;
                    
                manager
                    .get_connection()
                    .execute_unprepared("CREATE INDEX idx_benchmark_posts_category ON benchmark_posts (category)")
                    .await?;
                ...
                manager
                    .get_connection()
                    .execute_unprepared("CREATE INDEX idx_benchmark_posts_priority ON benchmark_posts (priority)")
                    .await?;
                ...
                manager
                    .get_connection()
                    .execute_unprepared("CREATE INDEX idx_benchmark_posts_author ON benchmark_posts (author)")
                    .await?;
            },
            _ => {
                ...
                manager
                    .get_connection()
                    .execute_unprepared("CREATE INDEX idx_benchmark_posts_title ON benchmark_posts (title)")
                    .await?;
            }
        }
...
async fn setup_benchmark_db(record_count: usize) -> Result<DatabaseConnection, sea_orm::DbErr> {
    ...
    for i in 0..record_count {
        let post = BenchmarkPostCreate {
            ...
            content: format!("This is benchmark content for post {}. It contains various keywords like performance, testing, database, queries, and optimization to test fulltext search capabilities.", i),
...
async fn benchmark_filtered_query(app: Router, filter: &str) -> Result<Vec<BenchmarkPost>, Box<dyn std::error::Error>> {
...
async fn benchmark_fulltext_search(app: Router, query: &str) -> Result<Vec<BenchmarkPost>, Box<dyn std::error::Error>> {
    let filter = format!("{{\"q\":\"{}\"}}", query);
    benchmark_filtered_query(app, &filter).await
}

// Benchmark sorted request
async fn benchmark_sorted_query(app: Router, sort_field: &str, order: &str) -> Result<Vec<BenchmarkPost>, Box<dyn std::error::Error>> {
    let uri = format!("/api/v1/benchmark_posts?sort={}&order={}", sort_field, order);
...
async fn benchmark_paginated_query(app: Router, page: usize, per_page: usize) -> Result<Vec<BenchmarkPost>, Box<dyn std::error::Error>> {
    let uri = format!("/api/v1/benchmark_posts?page={}&per_page={}", page, per_page);
...
async fn benchmark_create_post(app: Router, post_data: BenchmarkPostCreate) -> Result<BenchmarkPost, Box<dyn std::error::Error>> {
...
async fn benchmark_complex_query(app: Router) -> Result<Vec<BenchmarkPost>, Box<dyn std::error::Error>> {
...
fn bench_crud_operations(c: &mut Criterion) {
    ...
    for size in dataset_sizes {
        ...
        let mut group = c.benchmark_group(format!("CRUD Operations {} ({}records)", backend_name, size));
        group.measurement_time(Duration::from_secs(10));
        
        // Simple GET all benchmark
        group.bench_with_input(
            BenchmarkId::new("get_all", size),
            &size,
            |b, _| {
                b.iter(|| {
                    rt.block_on(black_box(benchmark_get_all(app.clone())))
                });
            },
        );
        ...
        for page_size in pagination_sizes {
            group.bench_with_input(
                ...
                |b, page_size| {
                    b.iter(|| {
                        rt.block_on(black_box(benchmark_paginated_query(app.clone(), 0, *page_size)))
                    });
                },
            );
        }
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    rt.block_on(black_box(benchmark_complex_query(app.clone())))
                });
            },
        );
...
fn bench_create_operations(c: &mut Criterion) {
    ...
    for initial_size in initial_sizes {
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    rt.block_on(black_box(benchmark_create_post(app.clone(), create_data.clone())))
                });
            },
        );
    }
